		...
		<div class="flex justify-between h-16">
			<div class="flex items-center">
				<a href="/" class="flex items-center gap-3">
					...
					<span class="font-bold text-xl text-surface-900">Extension Lab</span>
				</a>
			</div>
			...
			<div class="lg:hidden flex items-center">
				<button 
					...
					class="p-2 rounded-lg text-surface-600 hover:bg-surface-100"
				>
			...
			<div class="px-4 py-2 space-y-1">
				{#each navItems as item}
					<a 
						...
						class="block px-3 py-2 rounded-lg text-sm font-medium {page.url.pathname === item.href ? 'bg-primary-100 text-primary-700' : 'text-surface-600 hover:bg-surface-100'}"
					>
						<span class="mr-2">{item.icon}</span>
						{item.label}
					</a>
